...
int main()
{
	...
	for(int count=0 ; count<5 ; )
	{
		...
		ip_len = ip->ihl * 4;
		struct tcphdr *tcp_hdr = (struct tcphdr*)(buffer + ip_len + sizeof(struct ethhdr));
		//printf("test\n");

		//only get UDP packet & check ip address by using strcmp
		if((ip->protocol == IPPROTO_TCP && strcmp(inet_ntoa(dst.sin_addr), "140.120.15.176") != 0) || (ip->protocol == IPPROTO_TCP && strcmp(inet_ntoa(src.sin_addr), "140.120.15.176") != 0))
		{
			printf("packet %d:\n", count+1);
